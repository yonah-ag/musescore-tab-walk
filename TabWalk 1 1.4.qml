 ...
 *
 * By yonah_ag https://musescore.com/user/28842914
 *
 ...
 *  1.1.4 : 29 Sep 2024 - Use Element enumerator rather than hard coded values
 */
         ...
         while (seg) {
            ...
            if (seg.annotations && seg.annotations.length) {
               for (var aa in seg.annotations) {
                  ...
                  if (anno.type == Element.STAFF_TEXT) { // STAFF TEXT
                     ...
                     segInfo += ",fontStyle:," + anno.fontStyle;
                  }
                  else if (anno.type == Element.TEMPO_TEXT) {  // TEMPO
                     segInfo += "," + (60*anno.tempo) + " bpm";
                  }
                  else if (anno.type == Element.HARMONY) { // HARMONY
                     segInfo += "," + anno.text;
                  }
               }
            }
            for (var vv = 0; vv < curScore.ntracks; ++vv) {
               ...
               if (elm) {
                  ...
                  else if (elm.type == Element.REST) { // REST
